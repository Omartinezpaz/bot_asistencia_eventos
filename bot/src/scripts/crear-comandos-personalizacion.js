...
async function registrarComandosPersonalizacion() {
  try {
    console.log('Registrando comandos de personalización en el bot...');
    console.log('Token del bot:', process.env.BOT_TOKEN ? 'Disponible' : 'No disponible');
    
    // Lista de comandos a registrar
    const comandos = [
      {
        command: 'personalizar_organizacion',
        description: 'Personalizar logo, colores y mensajes de tu organización'
      }
    ];
...
// Ejecutar la función
registrarComandosPersonalizacion()
  .then(() => {
    console.log('\nProceso completado');
    process.exit(0);
  })
